...
4. **精炼命名原则（Backpack & Radial Friendly）**：
   小程序名称必须简练利落，**最长建议不超过 6 个字，强烈建议以 2 ~ 4 个字为最佳**（如“日历”、“截图”、“智能识别”、“计算器”）。
   - **设计缘由**：燕子核心交互组件【轮盘背包（Backpack）】与【燕环（RadialMenu）】采用了紧凑的圆形扇区与网格化槽位布局。过长的名称（如“任务栏日历 (Taskbar Calendar)”）会导致扇区内文字被迫换行、重叠挤压或被省略号截断；而 2~4 个字的精炼命名能够在轮盘和背包中优雅居中、浑然一体。
...
| `name` | `string` | 是 | 扩展用户可见名称。**规范建议最长不超过 6 个字，以 2~4 个字最佳**，完美适配轮盘背包与燕环呈现 | `"日历"` |
